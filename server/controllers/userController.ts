...
export const updateMe = asyncHandler(async (req: Request, res: Response) => {
  ...
  // ── Basic scalar fields ─────────────────────────────────────────────────
  const SCALAR = ['name', 'bio', 'department', 'branch', 'passout_year', 'gender', 'availability_status'] as const;
  ...
  if (body.skills !== undefined) {
    ...
    const toRemove = normaliseArray(body.removeSkills).map((s: string) => s.toLowerCase());
    if (toAdd.length || toRemove.length) {
      const current: string[] = (req.user as any).skills ?? [];
      let merged = [...current];
      if (toAdd.length)    merged = [...new Set([...merged, ...toAdd])];
      if (toRemove.length) merged = merged.filter((s) => !toRemove.includes(s.toLowerCase()));
  ...
  if (body.interests !== undefined) {
    ...
    const toRemove = normaliseArray(body.removeInterests).map((s: string) => s.toLowerCase());
    if (toAdd.length || toRemove.length) {
...
export const searchUsers = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (requestedSkills.length) {
    ...
    query.skills = { $in: requestedSkills.map((s) => new RegExp(`^${escapeRegex(s)}$`, 'i')) };
  }

  // ── Scalar filters ──────────────────────────────────────────────────────────
  if (req.query.gender)              query.gender              = req.query.gender;
  if (req.query.branch)              query.branch              = req.query.branch;
  if (req.query.passout_year)        query.passout_year        = Number(req.query.passout_year);
  if (req.query.availability_status) query.availability_status = req.query.availability_status;
