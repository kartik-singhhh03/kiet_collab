...
export const createTeam = asyncHandler(async (req: Request, res: Response) => {
  const { team_name, hackathon_name, required_skills,
          required_female_members, max_team_size } = req.body ?? {};

  if (!team_name?.trim())      return sendError(res, 'team_name is required');
  if (!hackathon_name?.trim()) return sendError(res, 'hackathon_name is required');
  ...
  if (isNaN(parsedFemale) || parsedFemale < 0)
    return sendError(res, 'required_female_members must be a non-negative number');

  if (parsedFemale >= parsedMax)
    return sendError(res, 'required_female_members must be less than max_team_size');
...
export const joinTeam = asyncHandler(async (req: Request, res: Response) => {
  const team = await Team.findById(req.params.teamId).populate('members', 'name avatar gender');
  ...
  if (alreadyMember) return sendError(res, 'You are already a member of this team', 400);

  // Max size check
  if (team.members.length >= team.max_team_size)
    return sendError(res, `Team is full (max ${team.max_team_size} members)`, 400);
  ...
  if (team.required_female_members > 0) {
    ...
    if (femaleStillNeeded > 0 && remainingSlots <= femaleStillNeeded && user.gender !== 'female')
      return sendError(
        res,
...
export const removeMember = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (String(req.params.userId) === String(team.created_by))
    return sendError(res, 'Team creator cannot be removed from the team', 400);

  // Target must be a member
  const isMember = team.members.some((m) => String(m) === String(req.params.userId));
  if (!isMember) return sendError(res, 'User is not a member of this team', 404);

  // Guard: removal must not break female quota
  if (team.required_female_members > 0) {
    const leavingUser = await User.findById(req.params.userId).select('gender').lean();
