...
router.post('/:id/register', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const already = event.registrations.find(r => r.user.toString() === req.user!._id.toString());
    if (already) return res.status(400).json({ error: 'Already registered' });

    event.registrations.push({ user: req.user!._id, registeredAt: new Date() } as any);
    await event.save();
...
router.post('/:id/submit', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const { title, description, repoUrl, demoUrl, screenshots, tags, teamId } = req.body;

    const team = await Team.findById(teamId);
    if (!team || team.event.toString() !== event._id.toString()) {
