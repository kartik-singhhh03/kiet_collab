...
router.post('/posts', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const post = await Question.create({ title, content, tags, author: req.user!._id });
...
// Answer a question
router.post('/posts/:id/answers', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
router.post('/posts/:id/accept/:answerId', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const post = await Question.findById(req.params.id);
    if (!post) return res.status(404).json({ error: 'Post not found' });
    ...
    const ans = post.answers.id(req.params.answerId);
...
// Vote question
router.post('/posts/:id/vote', authenticateToken, async (req: AuthRequest, res) => {
  try {
