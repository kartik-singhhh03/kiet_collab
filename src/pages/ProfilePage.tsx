...
import {
    Camera, Github, Linkedin, Globe, X,
    Check, Edit3, MapPin, BookOpen, Briefcase,
        ...
        >
            <div style={{ marginBottom: subtitle ? '0.3rem' : '1.5rem' }}>
                <h2 style={{
                    ...
                    color: B.dark, letterSpacing: '-0.02em', fontFamily: FONT,
                }}>{title}</h2>
                {subtitle && (
                    <p style={{ fontSize: '0.78rem', color: B.muted, fontFamily: FONT, marginTop: '0.2rem', marginBottom: '1.5rem' }}>
                        {subtitle}
                    </p>
                )}
            </div>
...
function FormField({ label, icon, children }: FieldProps) {
    return (
        <div className="pp-field" style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            <label style={{
                fontSize: '0.75rem', fontWeight: 700,
                color: B.muted, letterSpacing: '0.02em',
        ...
        <div style={{
            ...
            {prefix && (
                <span style={{
                    ...
                    fontSize: '0.83rem', color: B.muted, fontFamily: FONT,
            ...
            <input
                ...
                style={{
                    ...
                    fontSize: '0.875rem', color: B.dark,
                    fontFamily: FONT,
                }}
            />
        </div>
    ...
    return (
        <textarea
            ...
            style={{
                ...
                padding: '0.8rem 1rem',
                fontSize: '0.875rem', color: B.dark,
                transition: 'all 0.2s', lineHeight: 1.65,
                boxShadow: focused ? '0 0 0 3px rgba(40,41,44,0.05)' : 'none',
            }}
        />
    );
...
function TagInput({ tags, onChange, placeholder = 'Type & press Enter', maxTags = 20, suggestions = [] }: TagInputProps) {
        ...
        <div style={{ position: 'relative' }}>
            ...
            >
                {tags.map(tag => (
                    <span key={tag} className="pp-tag">
                        {tag}
                        <button className="pp-tag-del" onClick={e => { e.stopPropagation(); remove(tag); }}>
            ...
            {showSug && filtered.length > 0 && input.length > 0 && (
                <div style={{
                    ...
                    borderRadius: '0.75rem', boxShadow: '0 8px 24px rgba(40,41,44,0.10)',
                    overflow: 'hidden',
                }}>
            ...
            <p style={{ fontSize: '0.7rem', color: B.muted, fontFamily: FONT, marginTop: '0.4rem' }}>
                Press <kbd style={{ background: B.active, borderRadius: '0.3rem', padding: '0 0.35rem', fontSize: '0.7rem', fontFamily: 'monospace' }}>Enter</kbd> or <kbd style={{ background: B.active, borderRadius: '0.3rem', padding: '0 0.35rem', fontSize: '0.7rem', fontFamily: 'monospace' }}>,</kbd> to add · Backspace to remove
            </p>
        </div>
    );
}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: B.muted, fontFamily: FONT }}>
                    {value ? 'Visible to recruiters & teammates' : 'Not accepting teammate requests'}
                </span>
            </div>
        </div>
    );
...
═══════════════════════════════════════════════════ */
function AvatarUpload({ name, avatarUrl, onFile }: {
    name: string; avatarUrl?: string; onFile: (f: File) => void;
}) {
        ...
        <div style={{ position: 'relative', display: 'inline-block' }}>
            ...
            <div
                ...
                style={{
                    ...
                    fontSize: '2rem', fontWeight: 800, fontFamily: FONT,
            ...
            <div
                ...
                style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    cursor: 'pointer', boxShadow: '0 2px 6px rgba(40,41,44,0.25)',
...
const SKILL_SUGGESTIONS = [
    'React', 'TypeScript', 'Python', 'Node.js', 'Java', 'C++', 'Machine Learning',
    'Data Structures', 'Flutter', 'Django', 'FastAPI', 'MongoDB', 'PostgreSQL',
    'Docker', 'Kubernetes', 'AWS', 'TensorFlow', 'OpenCV', 'Next.js', 'Express.js',
    'Go', 'Rust', 'GraphQL', 'Redis', 'Figma', 'UI/UX', 'Android', 'iOS', 'Swift',
];
const INTEREST_SUGGESTIONS = [
    'Web Development', 'AI/ML', 'Cybersecurity', 'Blockchain', 'IoT', 'Robotics',
    'Data Science', 'Game Development', 'Open Source', 'Cloud Computing',
        ...
        <div style={{
            ...
            <select
                ...
                style={{
                    ...
                    fontSize: '0.875rem', color: value ? B.dark : B.muted,
                    fontFamily: FONT, cursor: 'pointer',
                }}
            >
                {options.map(o => <option key={o} value={o}>{o}</option>)}
            </select>
            <ChevronDown size={15} style={{ position: 'absolute', right: '0.85rem', color: B.muted, pointerEvents: 'none' }} />
        </div>
    );
}
...
export default function ProfilePage({ user }: { user: ProfileUser }) {
    ...
    const handleSave = async () => {
        ...
        await new Promise(r => setTimeout(r, 1100)); // Simulated API call
    ...
    return (
      ...
      `}</style>
        ...
        ══════════════════════════════════ */}
                <Section title="Profile Overview" subtitle="Your public-facing identity" delay={0}>
                    <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        ...
                        <AvatarUpload
                            ...
                            onFile={f => { setAvatarFile(f); setSaved(false); }}
                        />

                        {/* Info */}
                        <div style={{ flex: 1, minWidth: 220, display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>

                            {/* Name (read-only display + editable below) */}
                            <div>
                                <p style={{ fontSize: '1.35rem', fontWeight: 900, color: B.dark, letterSpacing: '-0.04em', fontFamily: FONT, marginBottom: '0.15rem' }}>
                                    {user.name}
                                </p>
                                <p style={{ fontSize: '0.82rem', color: B.muted, fontFamily: FONT, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                            ...
                            {/* Availability Toggle */}
                            <div>
                                <p style={{ fontSize: '0.68rem', fontWeight: 700, color: B.muted, letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: FONT, marginBottom: '0.6rem' }}>
                                    Status
                                </p>
                                <AvailabilityToggle
            ...
            2. BIO
        ══════════════════════════════════ */}
                <Section title="About Me" subtitle="Write a short bio that showcases who you are" delay={60}>
                    <FormField label="Bio" icon={<BookOpen size={12} />}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                            <span style={{ fontSize: '0.7rem', color: data.bio.length > 300 ? B.err : B.muted, fontFamily: FONT }}>
                                {data.bio.length}/320
                            </span>
                        </div>
                    </FormField>
            ...
            3. SKILLS + INTERESTS
        ══════════════════════════════════ */}
                <Section title="Skills & Interests" subtitle="Help teammates discover you based on your expertise" delay={120}>
            ...
            4. SOCIAL LINKS
        ══════════════════════════════════ */}
                <Section title="Social Links" subtitle="Connect your profiles so teammates can find you" delay={180}>
        ...
        ══════════════════════════════════ */}
                <div className="pp-fadein" style={{
                    ...
                    <div>
                        ...
                        <p style={{ fontSize: '0.75rem', color: B.muted, fontFamily: FONT, marginTop: '1px' }}>
                            {saved ? 'Your changes are live on your profile.' : 'Your changes will be visible to other students.'}
                        </p>
                    </div>
