...
import { Rocket, Eye, EyeOff, ArrowLeft, User, Mail, Lock, ChevronDown } from 'lucide-react';
...
/* ─── Helpers ─── */
const isKietEmail = (e: string) => /^[^\s@]+@kiet\.edu$/.test(e.trim().toLowerCase());
...
function Field({ label, error, children, required }: FieldProps) {
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            ...
            {error && (
                <span style={{
                    fontSize: '0.75rem', color: B.err,
                    borderBottom: `1.5px solid ${B.err}`,
                    paddingBottom: '2px', display: 'inline-block', width: 'fit-content',
                }}>
                    {error}
                </span>
            )}
...
function TextInput({ type = 'text', value, onChange, placeholder, error, icon, rightEl, autoComplete }: TextInputProps) {
    ...
    return (
        <div style={{
            ...
            borderRadius: '0.75rem', padding: '0 0.9rem',
            transition: 'all 0.2s', boxShadow: focused ? '0 0 0 3px rgba(40,41,44,0.05)' : 'none',
        }}>
            {icon && <span style={{ color: error ? B.err : focused ? B.dark : B.muted, flexShrink: 0, display: 'flex' }}>{icon}</span>}
            <input
                type={type}
                value={value}
...
function SelectInput({ value, onChange, options, placeholder, error, icon }: SelectInputProps) {
    const [focused, setFocused] = useState(false);
    return (
        <div style={{
            display: 'flex', alignItems: 'center', gap: '0.6rem',
            ...
            <ChevronDown size={16} style={{ color: B.muted, flexShrink: 0, pointerEvents: 'none' }} />
        </div>
    );
}
    ...
    return (
        <div style={{
            ...
            <a href="/" style={{
                ...
                <span style={{ fontSize: '1rem', fontWeight: 700, color: B.dark, letterSpacing: '-0.02em' }}>
                    KIET Collab
                </span>
            </a>
            ...
            {/* Bottom note */}
            <p style={{ marginTop: '1.5rem', fontSize: '0.75rem', color: B.muted, textAlign: 'center' }}>
...
function PrimaryBtn({ children, onClick, type = 'button', loading, disabled }: PrimaryBtnProps) {
    ...
    return (
        <button
            ...
            style={{
                ...
                borderRadius: '9999px', cursor: disabled ? 'not-allowed' : 'pointer',
            ...
            {loading ? (
                <>
                    <span style={{
                        ...
                        animation: 'authSpin 0.8s linear infinite', display: 'inline-block',
                    }} />
                    Please wait…
                </>
            ) : children}
        ...
        <>
            ...
            <PageShell>
                ...
                <button onClick={onGoLanding} style={{
                    ...
                    color: B.muted, fontSize: '0.82rem', fontFamily: 'Inter,sans-serif',
                ...
                <h1 style={{
                    ...
                    color: B.dark, letterSpacing: '-0.04em',
                    margin: '0 0 0.35rem',
                }}>
                    Welcome back
                </h1>
                ...
                <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
                        ...
                        <TextInput
                            ...
                            rightEl={
                                <button type="button" onClick={() => setShowPwd(p => !p)} style={{
                                    background: 'none', border: 'none', cursor: 'pointer',
                                    color: B.muted, display: 'flex', padding: '0 0.25rem',
                                    flexShrink: 0,
                                }}>
                    ...
                    {apiErr && (
                        <div style={{
                            ...
                            <p style={{ margin: 0, fontSize: '0.83rem', color: B.err }}>{apiErr}</p>
                        </div>
                    )}
                ...
                <div style={{
                    ...
                    <p style={{ fontSize: '0.85rem', color: B.muted, margin: 0 }}>
                        ...
                        <button onClick={onGoSignup} style={{
                            ...
                            color: B.dark, fontWeight: 700,
                            cursor: 'pointer', fontSize: '0.85rem',
                            fontFamily: 'Inter,sans-serif',
                            textDecoration: 'underline', textDecorationColor: 'rgba(40,41,44,0.25)',
...
export function SignupPage({ onSuccess, onGoLogin, onGoLanding }: SignupPageProps) {
    ...
    const [errors, setErrors] = useState<Partial<SignupForm & { confirm: string }>>({});
    const [apiErr, setApiErr] = useState('');
    const [loading, setLoading] = useState(false);
    ...
    const validate = () => {
        ...
        else if (!isKietEmail(form.email)) e.email = 'Must be a valid @kiet.edu email';
            ...
            <PageShell>
                ...
                <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            rightEl={
                                form.email && isKietEmail(form.email) ? (
                                    <span style={{
                                        ...
                                        color: B.success, background: 'rgba(56,161,105,0.09)',
                    ...
                    <Field label="Password" error={errors.password} required>
                        <TextInput
                            ...
                            rightEl={
                                <button type="button" onClick={() => setShowPwd(p => !p)} style={{
                                    background: 'none', border: 'none', cursor: 'pointer',
                                    color: B.muted, display: 'flex', padding: '0 0.25rem', flexShrink: 0,
                                }}>
                                    {showPwd ? <EyeOff size={16} /> : <Eye size={16} />}
                                </button>
                            }
                        ...
                        {form.password.length > 0 && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                                <div style={{ display: 'flex', gap: '4px', height: 3 }}>
                                    {[0, 1, 2].map(i => (
                                        <div key={i} style={{
                                            ...
                                            background: i < pwdStrength ? strengthColors[pwdStrength - 1] : 'rgba(40,41,44,0.1)',
                                ...
                                <span style={{
                                    fontSize: '0.72rem',
                                    color: pwdStrength > 0 ? strengthColors[pwdStrength - 1] : B.muted,
                                }}>
                    ...
                    <p style={{
                        ...
                        color: B.muted, margin: 0, lineHeight: 1.6,
                    }}>
                        By creating an account you agree to our{' '}
                        <span style={{ color: B.dark, fontWeight: 600, cursor: 'pointer', textDecoration: 'underline', textDecorationColor: 'rgba(40,41,44,0.22)' }}>
                            Terms of Service
                        </span>
                    </p>
                </form>
